...
export const createUser = async ( newUser: SignUpDTO, dispatch:AppDispatch, navigate: Function, token: any) =>{
...
export const createUserAdmin = async ( newUser: SignUpDTO, dispatch:AppDispatch, navigate: Function, token: any) =>{
    console.log(newUser, "dentro do actions do create")
    try {
        await api.post('/user/saveAdmin?role=2', newUser, api.defaults.headers.common['Authorization'] = token)
        const stateNewUser = {...newUser, type: 'CREATE_USER' }
        dispatch(stateNewUser)
        setupLoginAfterPost(newUser, dispatch, navigate)
    } catch (error) {
